...
本部分将详细介绍Nav2导航参数优化的测试策略、实验设计以及批量测试框架的实现。

%--------------------------------------------------------------------
\subsection{测试策略与实验设计}

\subsubsection{测试框架概述}

为了系统性地评估不同Nav2参数配置对导航性能的影响,本项目设计并实现了一套\textbf{自动化批量测试框架}。该框架采用分层架构,由调度层和执行层两部分组成:
...
\toprule
\textbf{层级} & \textbf{脚本} & \textbf{职责} \\
\midrule
调度层 & batch\_nav\_test.sh & 遍历参数配置、管理测试生命周期、汇总结果 \\
执行层 & launch\_tiago\_test.sh & 启动仿真环境、运行单次测试、采集性能数据 \\
\bottomrule
\end{tabular}
\end{table}
...
\subsubsection{参数化测试策略}

测试框架采用\textbf{参数覆盖机制}进行参数化管理。所有待测试的Nav2参数配置以YAML文件形式存放在\texttt{config/nav2\_params/}目录下:

\begin{lstlisting}[language=bash]
config/nav2_params/
|-- baseline.yaml        # 基准配置
|-- high_speed.yaml      # 高速导航配置
|-- conservative.yaml    # 保守配置
|-- low_inflation.yaml   # 低膨胀半径配置
|-- ...
\end{lstlisting}

每个配置文件采用\textbf{增量覆盖}的方式,仅需指定与基准参数不同的部分。系统通过\texttt{merge\_nav2\_params.py}工具自动将覆盖参数与基准参数合并,生成完整的Nav2配置文件。这种设计具有以下优势:

\begin{itemize}
    \item \textbf{简洁性}:配置文件只包含差异部分,易于阅读和维护
    \item \textbf{一致性}:所有测试共享相同的基准配置,确保对比的公平性
    \item \textbf{可追溯性}:清晰记录每次测试的参数变化
\end{itemize}

参数合并示例:
\begin{lstlisting}[language=Python]
# high_speed.yaml - 仅包含需要覆盖的参数
controller_server:
  ros__parameters:
    FollowPath:
...
\subsubsection{单次测试执行流程}

每次测试由\texttt{launch\_tiago\_test.sh}脚本执行,完整流程如下:

\begin{figure}[H]
\centering
\begin{tikzpicture}[
    node distance=0.6cm,
    box/.style={rectangle, draw, rounded corners, minimum width=10cm, minimum height=0.6cm, align=left, font=\small, fill=blue!5},
    arrow/.style={->, >=stealth, thick}
]
    \node[box] (step1) {1. 启动 Gazebo 仿真环境 (warehouse 世界)};
    \node[box, below=of step1] (step2) {2. 生成 TIAGo 机器人模型};
    \node[box, below=of step2] (step3) {3. 启动 ROS-Gazebo 桥接 (clock, cmd\_vel, odom, scan)};
    \node[box, below=of step3] (step4) {4. 发布 TF 变换 (odom$\to$base\_footprint, 激光 frame)};
    \node[box, below=of step4] (step5) {5. 启动 Nav2 导航栈 (带测试参数)};
    \node[box, below=of step5] (step6) {6. 设置初始位姿 (自动/手动)};
    \node[box, below=of step6] (step7) {7. 执行 waypoint 导航测试};
    \node[box, below=of step7] (step8) {8. 采集性能数据并生成报告};
...
测试脚本会自动统计waypoints配置文件中的目标点数量,并在测试开始前显示测试概要信息,包括测试名称、参数文件路径、输出目录等。

\subsubsection{测试控制参数}
...
\midrule
WAYPOINT\_TIMEOUT & 120s & 单个目标点的导航超时时间 \\
PAUSE\_BETWEEN & 10s & 测试间隔时间,确保进程完全清理 \\
HEADLESS & 0 & 无头模式(1=无GUI),适用于服务器环境 \\
RUN\_RVIZ & 1 & 是否启动RViz可视化 \\
SKIP\_EXISTING & 0 & 跳过已有结果的测试,支持断点续测 \\
OUTPUT\_DIR & test\_results/ & 测试结果输出目录 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{lstlisting}[language=bash]
# 无头模式批量测试,跳过已完成的测试
HEADLESS=1 RUN_RVIZ=0 SKIP_EXISTING=1 ./map/batch_nav_test.sh

# 只测试指定配置
./map/batch_nav_test.sh baseline high_speed conservative

# 设置每个waypoint的超时时间为60秒
WAYPOINT_TIMEOUT=60 ./map/batch_nav_test.sh
\end{lstlisting}

\subsubsection{容错与进程管理}

为确保批量测试的稳定性和可靠性,测试框架实现了完善的容错机制:

\paragraph{1. 信号捕获与优雅退出}

通过\texttt{trap}命令捕获INT和TERM信号,确保测试中断时能够正确清理所有相关进程:
...
清理函数会依次终止以下进程组:
\begin{itemize}
    \item Gazebo/Ignition相关进程(gz sim, gzserver, gzclient)
    \item Nav2导航相关进程(bt\_navigator, controller\_server, planner\_server等)
    \item 可视化和桥接进程(rviz2, parameter\_bridge, robot\_state\_publisher)
    \item 测试辅助进程(nav\_performance\_test, odom\_to\_tf, laser\_frame\_remapper)
\end{itemize}

\paragraph{2. 进程隔离}

每次测试结束后,无论成功与否,都会执行完整的进程清理流程:
\begin{enumerate}
    \item 首先发送SIGTERM信号,允许进程优雅退出
    \item 等待2秒后,对残留的Gazebo进程发送SIGKILL强制终止
    \item 在测试间隔期间等待指定时间(默认10秒),确保系统资源完全释放
\end{enumerate}

\paragraph{3. 状态记录与断点续测}

测试框架实时记录每个测试的执行状态到\texttt{batch\_summary.yaml}文件:
...
通过设置\texttt{SKIP\_EXISTING=1},可以跳过已存在结果的测试,实现断点续测功能。

\paragraph{4. 退出码传递}

单次测试的失败不会中断整个批量测试流程。框架会统计所有测试的通过/失败/跳过数量,最终根据是否有失败的测试返回相应的退出码。

\subsubsection{输出与结果分析}

测试结果按批次组织,采用统一的目录结构:

\begin{lstlisting}[language=bash]
test_results/
|-- batch_20241205_143000/
    |-- batch_summary.yaml           # 批量测试摘要
    |-- nav_test_baseline.yaml       # baseline配置详细结果
    |-- nav_test_baseline.csv        # baseline配置CSV格式结果
    |-- nav_test_high_speed.yaml     # high_speed配置详细结果
    |-- nav_test_high_speed.csv
    |-- nav_test_conservative.yaml
    |-- nav_test_conservative.csv
    |-- comparison_report.html       # 自动生成的对比报告
\end{lstlisting}

\paragraph{批量测试摘要}

\texttt{batch\_summary.yaml}文件记录整个批量测试的元信息和汇总结果:

\begin{lstlisting}[language=Python]
batch_id: "batch_20241205_143000"
start_time: "2024-12-05T14:30:00+08:00"
end_time: "2024-12-05T15:45:00+08:00"
configs_tested: 5
config_names:
  - baseline
  - high_speed
...
批量测试完成后,框架自动调用\texttt{compare\_nav\_results.py}脚本生成跨配置的性能对比报告。该报告以HTML格式输出,包含:
\begin{itemize}
    \item 各配置的导航成功率对比
    \item 平均导航时间和路径长度对比
    \item 速度和加速度统计对比
...
\subsubsection{测试设计原则}

本测试框架的设计遵循以下原则:

\begin{enumerate}
    \item \textbf{可重复性}:相同的配置在相同的环境下应产生一致的结果。通过固定随机种子、使用仿真时间等方式确保测试的可重复性。

    \item \textbf{可对比性}:所有测试使用相同的waypoints路线、相同的仿真环境、相同的初始条件,仅改变待测试的Nav2参数,确保对比的公平性。

    \item \textbf{可扩展性}:添加新的测试配置只需在\texttt{config/nav2\_params/}目录下创建新的YAML文件,无需修改测试框架代码。

    \item \textbf{自动化}:从测试启动到结果生成全程自动化,支持无人值守的批量测试,适合在CI/CD环境中运行。

    \item \textbf{健壮性}:完善的错误处理和进程管理机制,确保单个测试的失败不会影响整体测试流程。
\end{enumerate}

这套测试策略为后续的Nav2参数优化实验提供了可靠的数据基础,使我们能够系统性地评估不同参数配置对导航性能的影响。
